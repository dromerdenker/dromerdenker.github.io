...
class PathConfig:
    ...
    def create_path_for_file_output_tweets(self, year, month, format="html", kind="tweets", day="01", first_words="hello_world") -> str:
        ...
        return os.path.join(self.dir_output, f"{kind}-{format}", f"{year:04}", f"{year:04}-{month:02}-{day:02}-{first_words}.{format}")
...
def parse_tweets(username, users, html_template, paths: PathConfig):
   ...
   """
    ...
    # name tweets by date and first words
    ...
    for timestamp, md, html in tweets:
        ...
        s_md = "".join([l for l in md if l.isalpha() or l.isspace()])
        first_three_words = "-".join((s_md.lower().split())[:3])
        grouped_tweets[(dt.year, dt.month, dt.day, first_three_words)].append((md, html))

    for (year, month, day, first_three_words), content in grouped_tweets.items():
        # Write into *.md files
        md_string = '\n\n----\n\n'.join(md for md, _ in content)
        md_path = paths.create_path_for_file_output_tweets(year, month, format="md", day=day, first_words=first_three_words)
...
def find_archive():
    ...
    """
    if is_archive(
            '../../Downloads/twitter-2024-11-28-f89ae7aee597a091bde00ceb63996b9b601ac40a1b2c9509f8ae880b949c4cfa copy'):
        return '.'
...
<body>
    ...
    if not os.path.isfile(paths.file_tweet_icon):
        shutil.copy(
            '../../Downloads/twitter-2024-11-28-f89ae7aee597a091bde00ceb63996b9b601ac40a1b2c9509f8ae880b949c4cfa copy/assets/images/favicon.ico', paths.file_tweet_icon)
